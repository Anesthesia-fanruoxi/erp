      ...
      <el-table :data="tableData" v-loading="loading" stripe>
        ...
        <el-table-column label="角色" min-width="160">
          <template #default="{ row }">
            ...
            >{{ name }}</el-tag>
            <span v-if="!row.roleNames?.length" style="color: #c0c4cc; font-size: 12px">暂无角色</span>
          </template>
        </el-table-column>
...
const statusMap: Record<number, { label: string; type: 'warning' | 'success' | 'danger' }> = {
...
function handleEdit(row: any) {
  ...
  dialogForm.password = ''
  dialogForm.roleIds = row.roleIds || []
  dialogVisible.value = true
  fetchRoles()
}
...
async function handleSubmit() {
  ...
  await dialogFormRef.value.validate(async (valid) => {
    ...
    try {
      if (isEdit.value) {
        const { userName, ...data } = dialogForm
        if (!data.password) {
          const { password, ...rest } = data
          await updateUser(editId.value, rest)
        } else {
